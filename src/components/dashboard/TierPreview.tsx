 ...
 */
export const TierPreview: React.FC<TierPreviewProps> = ({ user, onSelectLevel }) => {
  return (
    <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 backdrop-blur-sm">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Level Tiers</h3>
          <p className="text-sm text-slate-400">Your journey through ISO</p>
        </div>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-hide">
        {levelTiers.map((tier) => {
          const status = getLevelStatus(tier.level, user);
          return (
            <TierCard
              key={tier.level}
              level={tier.level}
